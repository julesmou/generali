...
def transfo_ligne(n):
    liste_nombres = DataDict[n].split(';') # séparer la chaîne en une liste de nombres
    nombres_flottants = [float(x) for x in liste_nombres[:-2]] # convertir chaque élément de la liste en nombre flottant
    derniers_nombres = [float(x[:-1])/100 for x in liste_nombres[-2:]] # convertir les deux derniers éléments de la liste en nombres entiers
